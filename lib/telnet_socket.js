...
module.exports = class TelnetSocket extends Duplex {
    ...
    static commandBuffer(code, option, subNegotiation) {
        ...
        let sequence = [ Commands.IAC, code, isNaN(option.code) ? option : option.code ];
        if (Array.isArray(subNegotiation)) {
            sequence = sequence.concat(subNegotiation);
        }
    ...
    _init() {
        ...
        this.sb = {
            send :  {
                ...
                charset : (separator, charset) => {
                    const subNegotiation = [ SubNegotiationCommands.REQUEST ];

                    subNegotiation.push(separator.charCodeAt(0));

                    for (let k of charset.split('')) {
                        subNegotiation.push(k.charCodeAt(0));
                    }
                ...
                msp : (name, trigger='SOUND', volume=100, loop=1, priority=50, cont=1, type=false, url=false) => {
                    trigger = trigger.toUpperCase();

                    if (trigger != 'SOUND' && trigger != 'MUSIC') {
                        throw new Error(`msp: trigger must be 'SOUND' or 'MUSIC'`);
                    }

                    let str = `!!${type}(${name}`
                    if (volume != 100) str += ` V=${volume}`
                    if (loop != 1) str += ` L=${loop}`
                    if (priority != 50) str += ` P=${priority}`
                    if (cont != 1) str += ` C=${cont}`
                    if (type) str += ` T=${type}`
                    if (url) str += ` U=${url}`
                    ...
                    return this.command(
                        Commands.SB,
                        Options.MSP,
                        str.split('').map(k => k.charCodeAt(0))
                    );
                },

                gmcp : (message, data) => {
                    const subNegotiation = [];
                    for (let k of message.split('')) {
                        subNegotiation.push(k.charCodeAt(0));
                    }

                    if (data) {
                        // 32 is a space character to separate the message from the data
                        subNegotiation.push(32);
